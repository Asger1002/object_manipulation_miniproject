...
- AABB parsing support is present, but masking is currently force-disabled (loud warning if `--aabb` is provided).
...
Note: AABB is currently disabled in runtime policy, so this will emit a warning and run with full cloud.
```
...
- `--aabb xmin ymin zmin xmax ymax zmax`: currently accepted but ignored (AABB policy disabled).
